...
class SemanticBuildVersion2(workingDir: File, baseConfig: SemanticBuildVersionConfiguration) {

  import VersionComponent.*

  private val logger = LoggerFactory.getLogger(classOf[SemanticBuildVersion2])
...
object SemanticBuildVersion2 {

  def apply(
    workingDir: File,
    baseConfig: SemanticBuildVersionConfiguration = SemanticBuildVersionConfiguration()
  ): SemanticBuildVersion2 =
    new SemanticBuildVersion2(workingDir, baseConfig)
}
