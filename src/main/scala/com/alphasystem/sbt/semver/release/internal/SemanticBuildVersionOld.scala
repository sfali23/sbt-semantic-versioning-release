...
class SemanticBuildVersionOld(
  workingDir: File,
  config: SemanticBuildVersionConfiguration) {

  import SemanticBuildVersionOld.*
  ...
  private def filterTags(tags: Set[String]): Set[String] =
    tags
      ...
      .filter(tag => SemanticBuildVersionOld.VersionPattern.nonEmpty(tag))
      .filter(tag =>
...
object SemanticBuildVersionOld {
  ...
  ) = new SemanticBuildVersionOld(workingDir, config)
}
