Feature: New pre-release bumping scenarios
  ...
  @new-pre-release
  Scenario Outline: New pre-release without any previous pre-release version, with custom config, will create new pre-release (Force bump)
    Given Record main branch
    And Load semantic build config from ({forceBump=true,componentToBump=<bumpComponent>,newPreRelease=true,snapshot=<snapshotFlag>,preReleaseConfig={prefix=alpha,separator=-,startingVersion=1}})
    And Following annotated: <annotated> tags (v0.1.0) has been created
    And Branch 'test' is created and checked out
    When Make changes and commit with message: 'new pre-release creation'
    And Main branch is checked out
    ...
    Examples:
      | bumpComponent | annotated | expectedVersion            | snapshotFlag  |
      | PATCH         | true      | 0.1.1-alpha-1              | false         |
      | PATCH         | false     | 0.1.1-alpha-1              | false         |
      | PATCH         | true      | 0.1.1-alpha-1-SNAPSHOT     | true          |
      | PATCH         | false     | 0.1.1-alpha-1-SNAPSHOT     | true          |
      | MINOR         | true      | 0.2.0-alpha-1              | false         |
      | MINOR         | false     | 0.2.0-alpha-1              | false         |
      | MINOR         | true      | 0.2.0-alpha-1-SNAPSHOT     | true          |
      | MINOR         | false     | 0.2.0-alpha-1-SNAPSHOT     | true          |
      | MAJOR         | true      | 1.0.0-alpha-1              | false         |
      | MAJOR         | false     | 1.0.0-alpha-1              | false         |
      | MAJOR         | true      | 1.0.0-alpha-1-SNAPSHOT     | true          |
      | MAJOR         | false     | 1.0.0-alpha-1-SNAPSHOT     | true          |

  @new-pre-release
  Scenario Outline: New pre-release without any previous pre-release version, with custom config, will create new pre-release (Auto bump)
    Given Record main branch
    And Load semantic build config from ({snapshot=<snapshotFlag>,preReleaseConfig={prefix=alpha,separator=-,startingVersion=1}})
    And Following annotated: <annotated> tags (v0.1.0) has been created
    And Branch 'test' is created and checked out
    When Make changes and commit with message: '[new-pre-release] creation with [<bumpComponent>]'
    And Main branch is checked out
