...
sealed trait Scenario1 extends TestScenario {
  ...
  override def runScenario(repository: TestRepository, adapter: JGitAdapter): String = {
    repository
      ...
      .createTag(SemanticBuildVersion2(repository.workingDirectory, config).determineVersion.toStringValue())
...
sealed trait Scenario3 extends TestScenario {
  ...
  override def runScenario(repository: TestRepository, adapter: JGitAdapter): String =
    Try(
      repository.createTag(SemanticBuildVersion2(repository.workingDirectory, config).determineVersion.toStringValue())
    ) match {
